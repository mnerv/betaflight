# cmake/sources.cmake
#
# Common source file lists, translated from mk/source.mk.
# All paths are relative to BF_SRC_DIR (src/main) unless prefixed.
# Call bf_prefix_srcs() after including this file to get full paths.
#
# Sets (full absolute paths):
#   BF_PG_SRCS          - Parameter group sources
#   BF_COMMON_SRCS      - Always-compiled application sources
#   BF_RX_SRCS          - SPI receiver driver sources
#   BF_FLASH_SRCS       - External flash sources
#   BF_SDCARD_SRCS      - SD-card sources
#   BF_SPEED_OPT_SRCS   - Files that should be compiled with speed optimisation
#   BF_SIZE_OPT_SRCS    - Files that should be compiled with size optimisation
#
# The caller must have set BF_SRC_DIR and BF_LIB_DIR beforehand.

# ---------------------------------------------------------------------------
# Helper: prepend BF_SRC_DIR to each item in a list
# ---------------------------------------------------------------------------
function(_bf_srcdir_prefix OUT_VAR)
    set(_result)
    foreach(_f ${ARGN})
        list(APPEND _result "${BF_SRC_DIR}/${_f}")
    endforeach()
    set(${OUT_VAR} ${_result} PARENT_SCOPE)
endfunction()

# ---------------------------------------------------------------------------
# PG (parameter group) sources
# ---------------------------------------------------------------------------
_bf_srcdir_prefix(BF_PG_SRCS
    pg/adc.c
...
# ---------------------------------------------------------------------------
# COMMON_SRC — always compiled
# ---------------------------------------------------------------------------
_bf_srcdir_prefix(BF_COMMON_SRCS
    ...
    # Non-simulator hardware drivers
    drivers/bus_spi.c
...
# BMI270 Bosch API (in lib/main)
list(APPEND BF_COMMON_SRCS
    "${BF_LIB_DIR}/BoschSensortec/BMI270-Sensor-API/bmi270_maximum_fifo.c"
)

# ---------------------------------------------------------------------------
# RX (SPI receiver) sources
# ---------------------------------------------------------------------------
_bf_srcdir_prefix(BF_RX_SRCS
...
# ---------------------------------------------------------------------------
# Flash storage sources
# ---------------------------------------------------------------------------
_bf_srcdir_prefix(BF_FLASH_SRCS
...
# SD-card sources
# ---------------------------------------------------------------------------
_bf_srcdir_prefix(BF_SDCARD_SRCS
    drivers/sdcard.c
...
# FatFS (in lib/main)
set(BF_FATFS_DIR "${BF_LIB_DIR}/FatFS")
file(GLOB _FATFS_SRCS "${BF_FATFS_DIR}/*.c")
list(APPEND BF_SDCARD_SRCS ${_FATFS_SRCS})

# ---------------------------------------------------------------------------
# OLC library (in lib/main)
# ---------------------------------------------------------------------------
set(BF_OLC_DIR "${BF_LIB_DIR}/google/olc")
set(BF_OLC_SRCS "${BF_OLC_DIR}/olc.c")

# ---------------------------------------------------------------------------
# Speed-optimised sources (common; platform cmake adds more)
# ---------------------------------------------------------------------------
_bf_srcdir_prefix(BF_SPEED_OPT_SRCS
    common/encoding.c
    common/filter.c
    common/maths.c
    common/pwl.c
...
# ---------------------------------------------------------------------------
# Size-optimised sources (common; platform cmake adds more)
# ---------------------------------------------------------------------------
_bf_srcdir_prefix(BF_SIZE_OPT_SRCS
    drivers/accgyro/accgyro_mpu6050.c
    drivers/accgyro/accgyro_mpu6500.c
    drivers/accgyro/accgyro_spi_mpu6000.c
    drivers/accgyro/accgyro_spi_mpu6500.c
...
# OLC is also size-optimised
list(APPEND BF_SIZE_OPT_SRCS "${BF_OLC_SRCS}")
